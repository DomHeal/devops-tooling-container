...
RUN apt-get update && apt-get -y upgrade && apt-get install --no-install-recommends -y ${PACKAGES} && \
    ...
    add-apt-repository --yes --update ppa:ansible/ansible && apt-get update && apt-get install -y terraform packer vault ansible && \
    ...
    wget -q -O kubectx.tar.gz https://github.com/ahmetb/kubectx/releases/download/v0.9.4/kubectx_v0.9.4_linux_x86_64.tar.gz && tar -xvf kubectx.tar.gz && mv kubectx /usr/local/bin && chmod +x /usr/local/bin/kubectx && \
    wget -q -O kubens.tar.gz https://github.com/ahmetb/kubectx/releases/download/v0.9.4/kubens_v0.9.4_linux_x86_64.tar.gz && tar -xvf kubens.tar.gz && mv kubens /usr/local/bin && chmod +x /usr/local/bin/kubens && \
    wget -q -O kubectl-slice.tar.gz https://github.com/patrickdappollonio/kubectl-slice/releases/download/v1.1.0/kubectl-slice_1.1.0_linux_x86_64.tar.gz && tar -xvf kubectl-slice.tar.gz && mv kubectl-slice /usr/local/bin && chmod +x /usr/local/bin/kubectl-slice && \
    wget -q -O kubelinter.tar.gz https://github.com/stackrox/kube-linter/releases/download/0.2.5/kube-linter-linux.tar.gz && tar -xvf kubelinter.tar.gz && mv kube-linter /usr/local/bin/kube-linter && chmod +x /usr/local/bin/kube-linter && \
    ...
    curl -sL https://aka.ms/InstallAzureCLIDeb | bash && az extension add --name azure-devops && \
    ...
    curl -Lo skaffold https://storage.googleapis.com/skaffold/releases/latest/skaffold-linux-amd64 && install skaffold /usr/local/bin/ && \
    ...
    curl -Lo stern https://github.com/wercker/stern/releases/download/1.11.0/stern_linux_amd64 && mv stern /usr/local/bin/ && chmod +x /usr/local/bin/stern && \
    ...
    goss v && rm -rf goss.yaml && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
